...
LANGUAGES = {
    "EN": {
        "title": "AI Coder Studio (Hybrid v7)",
        "tab_new": "New Project",
        "tab_fav": "Favorites",
        "tab_hist": "History",
    ...
    "GR": {
        "title": "AI Coder Studio (Υβριδική v7)",
        "tab_new": "Νέο Project",
        "tab_fav": "Αγαπημένα",
        "tab_hist": "Ιστορικό",
...
class AIStudioApp:
    def __init__(self, root):
        ...
        # Φορτώνουμε τα μοντέλα χωρίς να πετάξουμε ακόμα μήνυμα σφάλματος
        ...
        # Ελέγχουμε αν το Ollama είναι κλειστό ΜΕΤΑ το άνοιγμα του παραθύρου
    ...
    def toggle_language(self):
        ...
        self.save_data()
        # Πριν το rebuild, σκανάρουμε για να έχουμε τα σωστά ονόματα κατηγοριών
        self.models_dict, self.all_models_flat = self.scan_and_categorize(silent=True)
    ...
    def setup_launcher_tab(self, frame):
        ...
        ttk.Entry(api_frame, textvariable=self.gemini_api_var, width=35, show="*").grid(row=0, column=1, padx=5, pady=5)
        ...
        ttk.Entry(api_frame, textvariable=self.groq_api_var, width=35, show="*").grid(row=1, column=1, padx=5, pady=5)
        ...
        self.path_var = tk.StringVar(); ttk.Entry(path_frame, textvariable=self.path_var, width=40).pack(side='left', expand=True, fill='x', padx=(0, 5))
        ...
        self.fav_name_var = tk.StringVar(); ttk.Entry(fav_frame, textvariable=self.fav_name_var, width=25).pack(side='left', padx=5)

        ttk.Button(frame, text=self.t("btn_launch"), command=self.launch_from_main).pack(pady=20, ipadx=20, ipady=10)

        # Binding AFTER all widgets created to avoid the TclError
    ...
    def update_models_dropdown(self, event=None):
        ...
        # Check if git_template_combo exists before setting
    ...
    def setup_favorites_tab(self, frame):
        self.fav_listbox = tk.Listbox(frame, width=70, height=20); self.fav_listbox.pack(pady=10, padx=10)
